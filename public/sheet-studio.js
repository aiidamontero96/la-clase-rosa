...
window.ROSA_SHEETS=(()=>{
  ...
  Object.assign(PLAY.themes,{
    Animales:[asset('cat','Gato'),asset('duck','Pato'),asset('rabbit','Conejo'),asset('fish','Pez'),asset('butterfly','Mariposa'),asset('dog','Perro')],
    Frutas:[asset('apple','Manzana'),asset('orange','Naranja'),asset('grapes','Uvas'),asset('banana','Plátano'),asset('strawberry','Fresa'),asset('pear','Pera')],
    Aula:[asset('pencil','Lápiz'),asset('books','Libros'),asset('scissors','Tijeras'),asset('teddy','Peluche'),asset('ball','Pelota'),asset('toy-car','Coche')],
    Naturaleza:[asset('seedling','Brote'),asset('tree','Árbol'),asset('tulip','Tulipán'),asset('herb','Planta'),asset('autumn-leaf','Hoja'),asset('sun','Sol')],
    'El tiempo':[asset('sun','Sol'),asset('cloud','Nube'),asset('partly-cloudy','Sol y nubes'),asset('rain','Lluvia'),asset('snowman','Muñeco de nieve'),asset('wind','Viento')],
    Emociones:[asset('happy','Contento'),asset('surprised','Sorprendido'),asset('sad','Triste'),asset('angry','Enfadado'),asset('worried','Preocupado'),asset('wave','Saludo')],
    Arte:[asset('pencil','Lápiz'),asset('books','Libros'),asset('scissors','Tijeras'),asset('paintbrush','Pincel'),asset('yarn','Lana'),asset('rosette','Adorno')],
    Investigación:[asset('magnifier','Lupa'),asset('shovel','Pala'),asset('dino-egg','Huevo'),asset('volcano','Volcán'),asset('notebook','Cuaderno'),asset('key','Llave')],
    Lugares:[asset('house','Casa'),asset('school','Colegio'),asset('playground','Parque'),asset('beach','Playa'),asset('stadium','Estadio'),asset('library','Biblioteca')],
  ...
  function thinkSheet(){const theme=PLAY.themes[ui.theme]||PLAY.themes.Naturaleza,other=PLAY.themes.Dinosaurios,at=i=>theme[(i+ui.seed)%theme.length],out=i=>{const item=at(i),alternatives=theme.filter(x=>x.src!==item.src);return alternatives[(i+ui.seed)%alternatives.length];};let body='';
  ...
  function geo(n,trace=false){const shapes=['<circle cx="40" cy="40" r="28"/>','<rect x="12" y="12" width="56" height="56"/>','<path d="M40 10 L70 68 H10 Z"/>'];return '<svg viewBox="0 0 80 80" class="worksheet-shape" role="img" aria-label="'+['círculo','cuadrado','triángulo'][n%3]+'"><g fill="none" stroke="#222" stroke-width="2"'+(trace?' stroke-dasharray="3 4"':'')+'>'+shapes[n%3]+'</g></svg>';}
  const dots=n=>'<span class="worksheet-dots">'+Array.from({length:n},()=>'<i></i>').join('')+'</span>';
  ...
  function shapeSheet(){let body='';
    if(ui.shapeType==='reconocer')body='<h2>RODEA LAS FORMAS IGUALES AL MODELO</h2><div class="shape-exercises">'+Array.from({length:4},(_,r)=>{const target=(r+ui.seed)%3;return '<div><strong>'+geo(target)+'</strong>'+[0,1,2,3,4].map((_,i)=>geo((i+r+ui.seed)%3)).join('')+'</div>';}).join('')+'</div>';
    else if(ui.shapeType==='continuar')body='<h2>DIBUJA LA FORMA QUE SIGUE</h2><div class="shape-exercises">'+Array.from({length:4},(_,r)=>'<div>'+[0,1,0,1].map(i=>geo((r+i+ui.seed)%3)).join('')+'<i></i><i></i></div>').join('')+'</div>';
    else body='<h2>REPASA Y DIBUJA OTRA AL LADO</h2><div class="shape-exercises">'+[0,1,2].map(i=>'<div>'+geo(i,true)+geo(i,true)+'<span class="complete-number-space"></span></div>').join('')+'</div><p>INVENTA UN DIBUJO CON ESTAS FORMAS.</p><div class="drawing-space"></div>';
    return header('Formas','Círculo, cuadrado y triángulo. Observo antes de dibujar.')+'<main>'+body+'</main>';
  }
