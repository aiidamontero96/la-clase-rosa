...
window.ROSA_AULA=(()=>{
  ...
  function projectHubs(){return window.ROSA_CLASSROOM.projectHubs();}
  function specialDaysHub(){return window.ROSA_CLASSROOM.specialDays();}
  function pdiGameList(selected){if(!selected.length)return '<div class="empty"><h3>Prepara tu selección</h3><p>Toca el corazón de los juegos que quieras tener a mano.</p>'+btn('Ver todos los juegos','pdi-filter','Todas')+'</div>';if(ui.filter!=='Todas')return gameTiles(selected);const order=['Matemáticas','Lenguaje','Lógica','Psicomotricidad','Proyecto','Juegos'];return order.map(name=>{const games=selected.filter(g=>area(g)===name);return games.length?'<section class="pdi-category"><div class="heading-row"><h2>'+E(name)+'</h2><span class="tag">'+games.length+' propuestas</span></div>'+gameTiles(games)+'</section>':'';}).join('');}
  function pdiPage(){const selected=GAMES.filter(g=>ui.filter==='Favoritos'?ui.book.favorites.includes(g[0]):ui.filter==='Todas'?area(g)!=='Asamblea':area(g)===ui.filter);return head('Pizarra digital','',ui.book.last?btn('↩ Continuar','pdi-resume','','secondary'):'','','')+'<div id="aula-save-status">'+statusHTML()+'</div>'+'<nav class="pdi-quick-nav" aria-label="Accesos de PDI"><a data-action="pdi-jump" data-value="pdi-asamblea" href="#pdi">☀ Asamblea</a><a data-action="pdi-jump" data-value="pdi-sesion" href="#pdi">▶ Mi sesión</a><a data-action="pdi-jump" data-value="pdi-habilidades" href="#pdi">◈ Juegos por habilidades</a><a href="#canciones">♪ Canciones</a></nav><div id="pdi-asamblea">'+assemblyHub()+'</div><div id="pdi-sesion">'+sessionHTML()+'</div><div id="pdi-habilidades">'+window.ROSA_CLASSROOM.skillsHub()+'</div><details class="pdi-teacher-tools"><summary>Elegir juegos para mi sesión</summary><div class="chips pdi-filter-chips">'+['Todas','Favoritos','Matemáticas','Lenguaje','Lógica','Asamblea','Proyecto','Juegos','Psicomotricidad'].map(name=>btn(name,'pdi-filter',name,'chip '+(ui.filter===name?'active':''))).join('')+'</div>'+pdiGameList(selected)+'</details>'+projectHubs()+specialDaysHub()+'<details class="pdi-later"><summary>Añadir más adelante</summary><p>Guarda nuevas canciones cuando las necesites.</p><a class="button secondary" href="#canciones">Canciones de la clase</a></details>';}
  ...
  function autoRead(text){clearTimeout(autoVoiceTimer);const p=state.projection;autoVoiceTimer=setTimeout(()=>{if(state.projection===p)speak(false,text,true);},120);}
  ...
  return {announce,ui,init,load,save,retry,level,isObjective,setup,launch,resume,next,setLevel,remember,cardTools,pdiPage,toolbar,footer,afterRender,won,custom,change,handle,close,stopVoice,feedbackVoice,workshop,printHTML,validMemory,validStoryPath,selectPiece,place,storyChoice,snapshot};
})();
