...
window.ROSA_AULA=(()=>{
  ...
  const levelNames=['Con ayuda','Practicamos','Un reto más','Nivel experto'];
  const pieceGames=['puzzle','ordenar-piezas','sombras','cestas','ordena-numeros','ordena-historia'];
  ...
  // Juegos cuya dificultad no cambia con el nivel: se oculta el selector de nivel en su proyección.
  const noLevelGames=['tamanos','posiciones','dino-patas','completa-dino'];
  const vocabGames=['puzzle','contar','falta','parejas','orden','memory','relaciona-vocabulario','tema-contar','tema-series','tema-vocabulario'];
  const vocabThemes=()=>Object.keys(window.ROSA_CLASSROOM?.vocabThemes||{});
  const isObjective=kind=>objectiveIds.includes(kind);
  const supportsLevel=kind=>isObjective(kind)&&!noLevelGames.includes(kind);
  ...
  function setup(p){p.level=p.payload?.missionWeek?p.payload.missionLevel:level();p.pattern=['AB','AB','ABC','ABC'][p.level-1];p.pairCelebration=false;p.drop=PLAY.pieces(p.kind,p.round,p.level,p.payload?.theme);p.storyId=PLAY.stories[p.round%PLAY.stories.length].id;p.storyPath=['inicio'];p.surpriseOpen=false;p.surpriseType='mezcla';p.surpriseRevealed=false;ui.selected=null;if(p.kind==='memory')resetMemory(p);}
  function resetMemory(p){const theme=p.payload?.theme,poolSize=theme?(window.ROSA_CLASSROOM.vocabThemes?.[theme]?.length||window.ROSA_CLASSROOM.themes[theme]?.length||6):9,pairs=Math.min(poolSize,[2,4,6,8][p.level-1]),values=Array.from({length:pairs},(_,i)=>(i+p.round)%poolSize);p.memory=shuffle([...values,...values]);p.matched=[];p.flipped=[];p.locked=false;p.pairCelebration=false;}
  ...
  async function toggleQueue(kind){if(!known(kind))return;await load();if(ui.book.queue.includes(kind))ui.book.queue=ui.book.queue.filter(x=>x!==kind);else if(ui.book.queue.length<8)ui.book.queue.push(kind);else{toast('La sesión tiene ocho juegos. Quita uno para añadir otro.');return;}saveSoon(['queue']);refreshPage();}
  ...
  async function startSession(){await load();if(!ui.book.queue.length){toast('Añade hasta ocho juegos a la sesión.');return;}await launch(ui.book.queue[0],{session:{games:[...ui.book.queue],index:0}});}
  ...
  function sessionHTML(){return '<section class="panel session-panel"><div class="heading-row"><h2>Mi sesión de PDI</h2><span class="tag">'+ui.book.queue.length+' / 8 juegos</span></div>'+(!ui.book.queue.length?'<p class="hint">Añade hasta ocho juegos con «+ A mi sesión». Después podrás pasar de uno a otro sin volver al menú.</p>':'<ol class="session-list">'+ui.book.queue.map((id,i)=>'<li><span><b>'+(i+1)+'</b> '+E(title(id))+'</span><div>'+btn('↑','queue-up',id,'quiet small')+btn('↓','queue-down',id,'quiet small')+btn('Quitar','queue-toggle',id,'quiet small')+'</div></li>').join('')+'</ol>')+'<div class="session-controls">'+levelSelect('aula-level')+'<button type="button" data-action="session-start"'+(!ui.book.queue.length?' disabled':'')+'>Empezar sesión →</button></div></section>';}
  ...
  function pdiNav(active='asamblea'){const links=[['asamblea','#pdi','☀','Asamblea'],['proyectos','#pdi-proyectos','🦕','Proyectos'],['dias','#pdi-dias','🎉','Días especiales'],['juegos','#pdi-juegos','◈','Todos los juegos'],['calendario','#pdi-calendario','📅','Calendario'],['canciones','#canciones','♪','Canciones']];return '<nav class="pdi-quick-nav pdi-section-nav" aria-label="Secciones de PDI">'+links.map(([id,href,icon,label])=>'<a class="'+(active===id?'active':'')+'" href="'+href+'"'+(active===id?' aria-current="page"':'')+'>'+icon+' '+label+'</a>').join('')+'</nav>';}
  ...
  function vocabSelect(p){if(!vocabGames.includes(p.kind))return '';const selected=p.payload?.theme||'Dinosaurios';return '<label class="aula-level aula-vocab" for="projection-vocab">Vocabulario <select id="projection-vocab">'+vocabThemes().map(theme=>'<option value="'+E(theme)+'"'+(theme===selected?' selected':'')+'>'+E(theme)+'</option>').join('')+'</select></label>';}
  function toolbar(p){if(p.kind==='calendario'&&p.payload?.externalCalendar)return '';return (window.ROSA_MISSIONS?.header(p)||'')+'<div class="aula-projection-controls">'+btn('🔊 Escuchar','speak','','secondary small')+btn('Parar voz','stop-voice','','quiet small')+(!p.payload?.missionWeek&&supportsLevel(p.kind)?levelSelect('projection-level'):'')+vocabSelect(p)+(ui.session?'<span class="tag">Sesión · '+(ui.session.index+1)+' / '+ui.session.games.length+'</span>':'')+voiceSettingsHTML()+'<span id="pdi-voice-status" class="sr-only" role="status"></span></div><div id="adventure-progress">'+(!p.payload?.missionWeek&&isObjective(p.kind)?adventureHTML():'')+'</div>';}
  ...
  function afterRender(body){const p=state.projection;if(!p)return;const previousSpeech=p.speak;p.speak=p.customSpeech||stripHTML((body.match(/<h2[^>]*>([\s\S]*?)<\/h2>/)||[])[1]||title(p.kind));const word=body.match(/class="pdi-word"[\s\S]*?<strong>([^<]+)<\/strong>/);if(word)p.speak+=' '+word[1]+'.';
    ...
    if(p.kind==='calendario'&&!p.payload?.externalCalendar)p.speak=p.date.toLocaleDateString('es-ES',{weekday:'long',day:'numeric',month:'long',year:'numeric'});
    if(p.kind==='calendario'&&p.payload?.externalCalendar){p.speak='';p.lastAutoSpeech='';stopVoice();remember();return;}
    ...
    if(p.answered&&p.answer!==null){const fb=$('#game-feedback');if(fb)fb.innerHTML=celebration(p.round,p.explanation||'',window.ROSA_PDI_REWARD?.praise(p.round)||'¡Muy bien!');}
    remember();
  }
  ...
  function renderPieces(p){
    ...
    else targets='<div class="drop-grid '+(game.kind==='puzzle'?'puzzle-grid':game.kind==='sombras'?'shadow-grid':['ordena-numeros','ordena-historia'].includes(game.kind)?'sequence-grid':'size-grid')+'" style="--cols:'+(game.cols||game.goal)+';--rows:'+(game.rows||1)+';--piece-ratio:'+(game.rows/game.cols||1)+'">'+game.pieces.map(piece=>{const placed=Object.values(game.placed).includes(piece.slot);return '<button type="button" class="pdi-slot '+(placed?'filled':'')+'" data-action="place-piece" data-value="'+piece.slot+'" data-slot="'+piece.slot+'" aria-label="Lugar '+(Number(piece.slot)+1)+'"'+(placed?' disabled':'')+(game.kind==='ordenar-piezas'?' style="--piece-size:'+piece.size+'px"':'')+'>'+(placed?PLAY.pieceArt(game,piece):PLAY.pieceSlotHint(game,piece))+'</button>';}).join('')+'</div>';
    return {body:'<h2>'+E(game.prompt)+'</h2><p class="hint piece-instruction">Arrastra, o toca una pieza y después su lugar.</p>'+(game.kind==='puzzle'?'<details class="puzzle-model"'+(p.level===1?' open':'')+'><summary>Ver modelo</summary>'+PLAY.picture(game.art.src,game.art.label)+'</details>':'')+'<div class="piece-workspace">'+targets+'<div class="piece-tray '+(game.kind==='puzzle'?'puzzle-grid':'')+'" style="--cols:'+(game.cols||3)+';--rows:'+(game.rows||1)+';--piece-ratio:'+(game.rows/game.cols||1)+'" aria-label="Piezas para colocar">'+tray.map(pieceButton).join('')+'</div></div><div id="piece-feedback" class="feedback" role="status" aria-live="polite">'+(complete?celebration(p.round,'',window.ROSA_PDI_REWARD?.praise(p.round)||'¡Muy bien!'):'')+'</div>',foot:btn('Otra propuesta →','projection-next')};
  }
  ...
  function place(slot){const p=state.projection,game=p?.drop;if(!game||p.answered)return;if(ui.selected===null){$('#piece-feedback').textContent='Primero toca una pieza.';return;}const piece=game.pieces.find(x=>x.id===ui.selected);if(!piece||piece.id in game.placed)return;if(piece.slot!==String(slot)||game.kind!=='cestas'&&Object.values(game.placed).includes(String(slot))){$('#piece-feedback').textContent='Mira con calma. Prueba otro lugar.';return;}game.placed[piece.id]=String(slot);ui.selected=null;const complete=Object.keys(game.placed).length>=game.goal;p.answered=complete;let message='';if(complete){const praise=window.ROSA_PDI_REWARD?.praise(p.round)||'¡Muy bien!';message=won(p,praise)||praise;}projectionRender();if(complete){$('#piece-feedback').innerHTML=celebration(p.round,'',message);window.ROSA_PDI_REWARD?.applause();window.ROSA_CLASSROOM?.burstConfetti?.();feedbackVoice(window.ROSA_PDI_REWARD?.praise(p.round)||'¡Muy bien!');$('#piece-feedback').scrollIntoView?.({block:'nearest'});}}
  function clearDrag(){const d=ui.drag;if(d?.ghost)d.ghost.remove();if(d?.target?.hasPointerCapture?.(d.pointer)){try{d.target.releasePointerCapture(d.pointer);}catch{}}ui.drag=null;}
  function pointerDown(event){const target=event.target.closest('[data-piece]');if(!target||event.button!==undefined&&event.button!==0||!state.projection?.drop)return;ui.drag={id:target.dataset.piece,pointer:event.pointerId,x:event.clientX,y:event.clientY,target,moved:false,projection:state.projection};try{target.setPointerCapture?.(event.pointerId);}catch{}}
  ...
  function pointerUp(event){const d=ui.drag;if(!d||d.pointer!==event.pointerId)return;const moved=d.moved,id=d.id,current=d.projection,target=[...document.elementsFromPoint(event.clientX,event.clientY)].map(node=>node.closest?.('[data-slot]')).find(Boolean);clearDrag();if(!moved||state.projection!==current)return;ui.ignoreClick=Date.now()+350;ui.selected=id;if(target)place(target.dataset.slot);else{projectionRender();$('#piece-feedback').textContent='La pieza sigue disponible. Puedes tocar su lugar.';}}
