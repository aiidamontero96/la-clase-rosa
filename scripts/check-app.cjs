...
function element(id){if(!elements.has(id))elements.set(id,{innerHTML:'',textContent:'',value:'',open:false,dataset:{},classList:{add(){},remove(){},toggle(){return false;}},setAttribute(){},addEventListener(){},showModal(){this.open=true;},close(){this.open=false;}});return elements.get(id);}
...
for(const file of ['data.js','pdi.js','play-content.js','sheet-studio.js','aula.js','rosi-config.js','rosi-media.js','rosi-episode.js','rosi-missions.js','app.js'])vm.runInContext(fs.readFileSync(path.join(dist,file),'utf8'),context,{filename:file});
...
assert.equal(data.challenges.length,50);assert.equal(data.resources.length,156);
assert.equal(new Set(data.resources.map(r=>r.id)).size,156);
const printables=data.resources.filter(r=>r.printable);assert.equal(printables.length,106);
const kusama=data.resources.filter(r=>r.project==='Yayoi Kusama');assert.equal(kusama.length,30);
const kusamaActivities=kusama.filter(r=>!r.printable),kusamaPrintables=kusama.filter(r=>r.printable);assert.equal(kusamaActivities.length,18);assert.equal(kusamaPrintables.length,12);
assert.equal(data.kusamaStages.length,3);assert.equal(new Set(data.kusamaStages.flatMap(s=>s[3])).size,18);
...
for(const r of kusamaActivities){assert(r.example,r.id+' example');assert(fs.existsSync(path.join(dist,'assets','kusama',r.id+'.webp')),r.id+' image');}
...
run("state.filter={age:'4',area:'',type:'',project:'Yayoi Kusama',q:''}");assert.equal(run("R.filter(r=>matches(r,'banco')).length"),30);assert(run("bank('banco')").includes('30 recursos'));
...
run("state.filter.type='PDI'");assert(run("bank('banco')").includes('42 recursos'));
run("state.filter.area='Matemáticas'");assert(run("bank('banco')").includes('10 recursos'));
...
const newGames=run('window.ROSA_PDI.games.map(g=>g[0])');assert.equal(newGames.length,22);assert.equal(run('new Set(GAMES.map(g=>g[0])).size'),42);
...
console.log(JSON.stringify({resources:156,kusamaActivities:18,kusamaPrintables:12,coursePrintables:22,printables:106,coloringPrintables:printables.filter(r=>r.coloring).length,questions:100,challenges:50,pdiModes:42,newPdiGames:22,patternCombinations:120,checks:'passed'}));
