...
for(const f of ['data.js','pdi.js','play-content.js','sheet-content.js','sheet-studio.js','aula.js','classroom.js','app.js'])run(fs.readFileSync(root+'/'+f,'utf8'));
const texts=new Map();
...
for(const kind of run('window.ROSA_PDI.games.map(g=>g[0])'))for(let lev=1;lev<=4;lev++)for(let n=0;n<360;n++){const r=run('window.ROSA_PDI.round('+JSON.stringify(kind)+','+n+',true,'+lev+')');if(r?.explanation)add(r.explanation,'feedback');}
